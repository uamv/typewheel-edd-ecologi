...
if ( ! class_exists( 'EDD_Client_Init' ) ) :
    ...
    class EDD_Client_Init {
        ...
        */
        public function __construct( $plugin_path, $api_url, $plugin_pages = [] ) {
        ...
        */
        ...
        {
            wp_register_script( 'edd-client', plugins_url( 'script.js', __FILE__ ), [ 'jquery', 'wp-util' ] );
        ...
        */
        ...
        {
            if (empty( $operation = $_POST['data']['operation'] ) || wp_verify_nonce($_POST['data']['nonce'], $this->plugin['machine_name'].'-edd-client-operations') === false) {
                wp_send_json_error( [ 'success' => false, 'data' => 'Something went wrong' ] );
            }

            switch($operation){
                case 'activate_license':
                $license = !empty($_POST['data']['license']) ? $_POST['data']['license'] : wp_send_json_error( [ 'success' => false, 'data' => 'License field can not be empty' ] );
                ...
                if( $license_data->license === 'valid' ) {
                    ...
                    wp_send_json_success( [ 'success' => true, 'data' => 'License successfully activated' ] );
                }
                break;
                ...
                if( $license_data->license === 'deactivated' || $license_data->license === 'failed' ) {
                    ...
                    wp_send_json_success( [ 'success' => true, 'data' => 'License deactivated for this site' ] );
                }
                break;
                case 'change_license':
                $new_license = !empty($_POST['data']['license']) ? $_POST['data']['license'] : wp_send_json_error( [ 'success' => false, 'data' => 'License field can not be empty' ] );
                ...
                if($new_license !== $old_license){
                    ...
                    if ($license_data->license === 'valid'){
                        ...
                        if($license_data->license === 'deactivated' || $license_data->license === 'failed'){
                            ...
                            wp_send_json_success( [ 'success' => true, 'data' => 'License Successfully Changed.' ] );
                        }
                    }
                }
                else{
                    wp_send_json_error( [ 'success' => false, 'data' => 'Enter a new license.' ] );
                }
                break;
                case 'check_expiry':
                $license = $this->plugin['license'];
        ...
        */
        public function check_expiry($license, $plugin_name, $api_url) {
            ...
            if( $license_data->license == 'valid' ) {
                wp_send_json_success( [ 'success' => true, 'data' => 'License will expire on: '.sanitize_text_field($expires) ] );
            }
            elseif ($license_data->license == 'expired'){
                wp_send_json_success( [ 'success' => true, 'data' => 'License expired on: '.sanitize_text_field($expires) ] );
            }
            elseif ($license_data->license == 'disabled'){
                wp_send_json_success( [ 'success' => true, 'data' => 'Your license has been disabled by the seller' ] );
            }
            elseif ($license_data->license == 'invalid'){
                wp_send_json_success( [ 'success' => true, 'data' => 'Invalid license key' ] );
            }
            else{
                wp_send_json_error( [ 'success' => false, 'data' => 'Something went wrong' ] );
            }
